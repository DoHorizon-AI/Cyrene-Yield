...
# training/core/src/cy_exec/training

训练契约、runtime 协调、引擎、执行器、checkpoint 与校验。

## 文件

| 条目 | 职责 |
|---|---|
| training/core/src/cy_exec/training/checkpoint/ | checkpoint 校验、摘要与制品发布。 |
| training/core/src/cy_exec/training/contracts/ | 训练意图、attempt、工作负载、事件和制品的稳定 Product 契约。 |
| training/core/src/cy_exec/training/engines/ | 训练引擎适配边界。 |
| training/core/src/cy_exec/training/executors/ | 本地、Kernel 与 Plugin 执行边界。 |
| training/core/src/cy_exec/training/validation/ | Product 准入不变量与 dataset-validator Plugin 适配器。 |
| training/core/src/cy_exec/training/worker/ | 嵌套训练模块。 |
| training/core/src/cy_exec/training/__init__.py | 包初始化与公开导入边界。 |
| training/core/src/cy_exec/training/artifacts.py | Python 实现或测试模块：artifacts.py。 |
| training/core/src/cy_exec/training/capability_seam.py | Python 实现或测试模块：capability_seam.py。 |
| training/core/src/cy_exec/training/control_plane.py | Python 实现或测试模块：control_plane.py。 |
| training/core/src/cy_exec/training/preflight.py | Python 实现或测试模块：preflight.py。 |
| training/core/src/cy_exec/training/runtime.py | Python 实现或测试模块：runtime.py。 |
| training/core/src/cy_exec/training/tiny_dry_run.py | Python 实现或测试模块：tiny_dry_run.py。 |

## 推荐阅读顺序

先从 training/core/src/cy_exec/training/checkpoint/ 开始，再沿包入口或导入关系继续阅读。
按 runtime 顺序阅读同级模块；职责边界参见父目录 README。
