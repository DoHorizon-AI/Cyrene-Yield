...
class KernelClient:
    ...
    def call(self, service: str, method: str, payload: dict[str, Any], *, timeout: float = 15) -> dict[str, Any]:
        """Invoke a canonical unary RPC without introducing another schema. | 调用原协议。"""
    ...
    def authority(self, method: str, payload: dict[str, Any]) -> dict[str, Any]:
        """Call the existing Lease/Worker authority. | 调用既有 Lease/Worker 权威。"""
    ...
    def capabilities(self) -> dict[str, Any]:
        """Read current Kernel inventory; online is not serving readiness. | 读取节点事实。"""
        return self.call("KernelService", "GetKernelCapabilities", {})

    def encode(self, name: str, payload: dict[str, Any]) -> bytes:
        """Encode a canonical message for the Rust placement adapter. | 编码原协议。"""
    ...
    def worker_control(self, outgoing: Iterable[dict[str, Any]]) -> Iterator[dict[str, Any]]:
        """Connect the worker's actual heartbeat/drain stream. | Worker 心跳与排空通路。"""
    ...
    def close(self) -> None:
        """Release the client channel, never the Kernel-owned Lease. | 关闭客户端通路。"""
        self.channel.close()
